...
/** 1 Energy, no Power. */
const CHEAP = makeUnit(1, ["fury"], {
  id: cardId("T-001"),
  name: "Cheap",
  cost: cost(1),
});
/** 2 Energy + 1 Fury: three Runes in total. */
const MID = makeUnit(3, ["fury"], {
  id: cardId("T-002"),
  name: "Mid",
  cost: cost(2, "fury"),
});
...
const FREE_SPELL = makeSpell(["fury"], {
  id: cardId("T-004"),
  name: "Free Spell",
  cost: cost(0),
});
...
describe("drawing a specific card", () => {
  it("matches a hand-computed opening-hand figure", () => {
    ...
    expect(
      probabilityOfCard({ deck: DECK, card: CHEAP.id, turn: 0 }),
    ).toBeCloseTo(1 - 35 / 210, 12);
  });
  ...
  it("is certain once the whole deck is seen", () => {
    expect(
      probabilityOfCard({ deck: DECK, card: CHEAP.id, turn: 20 }),
    ).toBeCloseTo(1, 12);
  });

  it("is impossible for a card that is not in the deck", () => {
    expect(
      probabilityOfCard({ deck: DECK, card: cardId("T-999"), turn: 5 }),
    ).toBe(0);
  });

  it("handles a requirement of several copies", () => {
    const one = probabilityOfCard({
      deck: DECK,
      card: CHEAP.id,
      turn: 2,
      copies: 1,
    });
    const two = probabilityOfCard({
      deck: DECK,
      card: CHEAP.id,
      turn: 2,
      copies: 2,
  ...
  it("reports expected copies", () => {
    ...
    expect(expectedCopies({ deck: DECK, card: CHEAP.id, turn: 0 })).toBeCloseTo(
      1.2,
      12,
    );
  });
...
describe("cost curve", () => {
  ...
  it("throws on a card the registry does not know", () => {
    expect(() =>
      costCurve([{ card: cardId("T-999"), count: 1 }], REGISTRY),
...
describe("opening hand", () => {
  ...
  it("answers a threshold question for a type by turn", () => {
    expect(probabilityOfType(DECK, REGISTRY, "spell", 1, 0)).toBeCloseTo(
      0.4,
      12,
    );
...
describe("analyzeDeck", () => {
  ...
  it("needs no engine, no agent and no randomness", () => {
    ...
    ).toEqual(JSON.stringify(analyzeDeck(DECK, REGISTRY).curve.averageEnergy));
  });

  it("throws on an unknown card rather than reporting a wrong number", () => {
    const broken: Deck = {
      ...DECK,
      main: [{ card: cardId("T-999"), count: 1 }],
    };
