...
describe('exactly', () => {
  ...
  it('matches a hand-computed value', () => {
    ...
    expect(exactly({ population: 40, successes: 3, draws: 5 }, 1)).toBeCloseTo(expected, 12);
  });
...
describe('atLeast', () => {
  it('matches the textbook opening-hand figure', () => {
    ...
    expect(atLeast({ population: 60, successes: 4, draws: 7 }, 1)).toBeCloseTo(0.3995, 4);
  });
  ...
  it('matches a hand-computed 40-card figure', () => {
    ...
    expect(atLeast({ population: 40, successes: 3, draws: 5 }, 1)).toBeCloseTo(0.3376, 4);
  });
  ...
  it('is certain when every card is a success', () => {
    expect(atLeast({ population: 10, successes: 10, draws: 4 }, 4)).toBeCloseTo(1, 12);
  });
  ...
  it('agrees with a direct sum of the exact probabilities', () => {
    ...
    for (const minimum of [1, 2, 3, 4]) {
      let direct = 0;
      for (let k = minimum; k <= Math.min(params.successes, params.draws); k += 1) {
...
describe('atMost and mean', () => {
  ...
  it('computes the expected count', () => {
    expect(mean({ population: 40, successes: 4, draws: 10 })).toBeCloseTo(1, 12);
...
describe('parameter validation', () => {
  it('rejects more successes than population', () => {
    expect(() => exactly({ population: 10, successes: 11, draws: 5 }, 1)).toThrow(RangeError);
  });

  it('rejects more draws than population', () => {
    expect(() => exactly({ population: 10, successes: 5, draws: 11 }, 1)).toThrow(RangeError);
  });

  it('rejects negative and fractional parameters', () => {
    expect(() => exactly({ population: -1, successes: 0, draws: 0 }, 0)).toThrow(RangeError);
    expect(() => exactly({ population: 10, successes: 2.5, draws: 5 }, 1)).toThrow(RangeError);
  });
});
...
describe('multivariateAtLeast', () => {
  ...
  it('matches a hand-computed four-draw figure', () => {
    ...
    expect(multivariateAtLeast([6, 6], [1, 1], 4)).toBeCloseTo(1 - (2 * 15) / 495, 12);
  });
  ...
  it('handles more than two categories', () => {
    ...
    expect(multivariateAtLeast([4, 4, 4], [1, 1, 1], 3)).toBeCloseTo(expected, 12);
  });
