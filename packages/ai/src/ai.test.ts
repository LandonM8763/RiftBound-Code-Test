...
const TWO_ACTIONS: readonly Action[] = [
  { type: "resolvePhase" },
  { type: "endTurn" },
];
...
describe("RandomAgent", () => {
  it("only ever returns an action it was offered", () => {
    ...
    for (let i = 0; i < 200; i += 1) {
      expect(
        isOffered(agent.chooseAction(view, TWO_ACTIONS), TWO_ACTIONS),
      ).toBe(true);
    }
  });
  ...
  it("is reproducible for a given seed", () => {
    ...
    const run = (): string[] => {
      ...
      return Array.from(
        { length: 50 },
        () => agent.chooseAction(view, TWO_ACTIONS).type,
      );
    };
...
describe("FirstActionAgent", () => {
  it("always takes the first offered action", () => {
    ...
    expect(agent.chooseAction(someView(), TWO_ACTIONS)).toEqual({
      type: "resolvePhase",
    });
  });

  it("throws when offered nothing", () => {
    expect(() => new FirstActionAgent().chooseAction(someView(), [])).toThrow(
      /no actions/,
    );
  });
});
...
describe("agent isolation", () => {
  it("hands the agent a view, never the game state", () => {
    ...
    playGame({
      registry: REGISTRY,
      decks: DECKS,
      agents: [spy, new FirstActionAgent()],
      seed: "spy",
      config: { maxTurns: 4 },
    });
...
describe("playGame", () => {
  ...
  it("is reproducible for the same game and agent seeds", () => {
    ...
    expect(JSON.stringify(first.finalState)).toEqual(
      JSON.stringify(second.finalState),
    );
  });
  ...
  it("reports events in order, starting with the game starting", () => {
    ...
    expect(
      events.filter((event) => event.type === "cardDrawn").length,
    ).toBeGreaterThan(0);
  });
...
describe("deck to engine integration", () => {
  ...
  it("draws the opening hand from the parsed 40-card deck", () => {
    ...
    for (const player of state.players) {
      ...
      expect(player.zones.mainDeck).toHaveLength(
        40 - state.config.openingHandSize,
      );
