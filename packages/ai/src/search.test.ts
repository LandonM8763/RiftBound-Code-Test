...
describe("SearchAgent strength", () => {
  ...
  function duel(
    games: number,
    a: (seed: string) => SearchAgent | HeuristicAgent,
    ...
    const margin =
      decided === 0 ? 1 : 1.96 * Math.sqrt((rate * (1 - rate)) / decided);
  ...
  it("beats the heuristic agent over a large sample", () => {
    const result = duel(
      40,
      (seed) =>
        new SearchAgent({ knowledge: KNOWLEDGE, worlds: 2, rollout: 20, seed }),
  ...
  it("measures itself as an even matchup, so the duel above means something", () => {
    ...
    const result = duel(
      ...
      (seed) =>
        new SearchAgent({ knowledge: KNOWLEDGE, worlds: 1, rollout: 10, seed }),
    );
