...
function duel(
  ...
  const margin =
    decided === 0 ? 1 : 1.96 * Math.sqrt((winRate * (1 - winRate)) / decided);
  ...
  it("plays deterministically for a given seed", () => {
    const play = (): number[] => {
      ...
      playGame({
        ...
        decks: [DECK, DECK],
        agents: [
          new HeuristicAgent({ seed: "fixed" }),
          new RandomAgent("fixed-opponent"),
        ],
        seed: "determinism",
  ...
  it("never chooses an action that was not offered", () => {
    ...
    expect(() =>
      playGame({
        ...
        decks: [DECK, DECK],
        agents: [
          new HeuristicAgent({ seed: "a" }),
          new HeuristicAgent({ seed: "b" }),
        ],
        seed: "mirror",
        config: { maxTurns: 300 },
...
describe("position evaluation", () => {
  ...
  it("ranks points above everything else (rule 323.1)", () => {
    ...
    expect(agent.evaluate(withPoint)).toBeGreaterThan(
      agent.evaluate(view) + DEFAULT_WEIGHTS.control,
    );
  });
...
describe("action preferences", () => {
  ...
  const choose = (actions: readonly Action[]): Action =>
    agent.chooseAction(view, actions);
  ...
  it("takes the only action when there is just one", () => {
    expect(choose([{ type: "resolvePhase" }])).toEqual({
      type: "resolvePhase",
    });
  });
...
describe("weights", () => {
  it("can be overridden without touching the scoring code", () => {
    const indifferent = new HeuristicAgent({
      seed: "w",
      weights: { control: 0, presence: 0 },
    });
    const view = viewOf();
    const held = {
      ...view,
