...
import { headToHead, simulate, type Entrant, type GameSummary } from './simulate.js';
...
const base = { decks: [DECK, DECK], registry: REGISTRY, config: { maxTurns: 300 } } as const;

describe('simulate', () => {
  it('reports wins, a rate and an interval per entrant', () => {
    const result = simulate({ ...base, entrants: [heuristic(), random()], games: 40, seed: 's' });
    ...
    const totalWins = result.entrants.reduce((sum, entrant) => sum + entrant.wins, 0);
  ...
  it('is reproducible from its seed', () => {
    const run = () =>
      simulate({ ...base, entrants: [heuristic(), random()], games: 20, seed: 'repeat' });
    expect(run()).toEqual(run());
  });

  it('gives different seeds different batches', () => {
    const a = simulate({ ...base, entrants: [heuristic(), random()], games: 20, seed: 'a' });
    const b = simulate({ ...base, entrants: [heuristic(), random()], games: 20, seed: 'b' });
  ...
  it('summarizes game length and points', () => {
    const result = simulate({ ...base, entrants: [heuristic(), random()], games: 20, seed: 'len' });
    ...
    const best = Math.max(...result.entrants.map((entrant) => entrant.points.max));
...
describe('seat rotation', () => {
  it('attributes a win to the entrant, not the seat it sat in', () => {
    ...
    const result = simulate({ ...base, entrants: [heuristic(), random()], games: 60, seed: 'rot' });
...
describe('headToHead', () => {
  it('measures the heuristic as better than a coin flip against random', () => {
    ...
    expect(excludes(challenger?.interval ?? { low: 0, high: 1, level: 0.95 }, 0.5)).toBe(true);
  ...
  it('measures two copies of the same agent as a coin flip', () => {
    ...
    expect(excludes(result.entrants[0]?.interval ?? { low: 0, high: 1, level: 0.95 }, 0.5)).toBe(
      false,
    );
  });
});
...
describe('bad input', () => {
  ...
  it('rejects a non-positive game count', () => {
    for (const games of [0, -1, 1.5]) {
      expect(() =>
        simulate({ ...base, entrants: [heuristic(), random()], games, seed: 'x' }),
