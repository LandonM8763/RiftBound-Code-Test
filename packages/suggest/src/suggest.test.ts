...
const NEUTRAL_UNIT = makeUnit(2, [], { id: cardId('S-012'), name: 'Neutral', cost: cost(1) });
...
const FILLER = Array.from({ length: 14 }, (_, i) =>
  makeUnit(2, [], { id: cardId(`S-3${String(i).padStart(2, '0')}`), name: `Filler ${i}`, cost: cost(1) }),
);
...
function fillerFor(count: number): { card: CardDefinition['id']; count: number }[] {
...
const nameOf = (card: Parameters<typeof REGISTRY.get>[0]) => REGISTRY.get(card)?.name ?? String(card);
...
describe('the objective', () => {
  it('scores a deck whose Runes match its demand above one that does not', () => {
    ...
    expect(matched.components['domains']).toBeGreaterThan(mismatched.components['domains'] ?? 0);
  });
  ...
  it('penalizes a card the Rune deck can never pay for', () => {
    ...
    const stuck = CONSISTENCY.score(
      deck({ main: mergeEntries([{ card: UNPAYABLE.id, count: 3 }, ...fillerFor(37)]) }),
      REGISTRY,
    );

    expect(stuck.components['curve']).toBeLessThan(fine.components['curve'] ?? 0);
  });
...
describe('applying an edit', () => {
  it('never mutates the deck it is given', () => {
    ...
    applyEdit(before, { kind: 'cut', list: 'main', card: NEUTRAL_UNIT.id, count: 1 });
  ...
  it('drops an entry that reaches zero rather than leaving a zero count', () => {
    ...
    expect(edited.main.find((entry) => entry.card === FURY_UNIT.id)).toBeUndefined();
  });
...
describe('suggesting edits', () => {
  ...
  it('states the measurement, not just that a number improved', () => {
    ...
    expect(report.suggestions[0]?.reason).toMatch(/50% of your Runes are calm/i);
  ...
  it('says nothing when the deck is already right on every axis', () => {
    const tuned = deck({ runes: mergeEntries([{ card: FURY_RUNE.id, count: 12 }]) });
  ...
  it('converges: applying the top suggestion repeatedly reaches a fixed point', () => {
    ...
    for (let round = 0; round < 20; round += 1) {
      const report = suggestEdits(current, REGISTRY, { format: CONSTRUCTED_BO1, limit: 1 });
    ...
    expect(suggestEdits(current, REGISTRY, { format: CONSTRUCTED_BO1 }).suggestions).toEqual([]);
  });
  ...
  it('ranks by how much the objective moved', () => {
    const report = suggestEdits(deck(), REGISTRY, { format: CONSTRUCTED_BO1, limit: 10 });
  ...
  it('keeps the structurally different suggestion rather than variations', () => {
    ...
    const pool = [CALM_UNIT.id, NEUTRAL_UNIT.id, ...FILLER.map((card) => card.id)];
    const report = suggestEdits(deck(), REGISTRY, {
      format: CONSTRUCTED_BO1,
      pool,
      limit: 20,
  ...
  it('honours the limit', () => {
    const report = suggestEdits(deck(), REGISTRY, { format: CONSTRUCTED_BO1, limit: 1 });
...
describe('candidates from a pool', () => {
  it('proposes nothing to add when given no pool', () => {
    ...
    expect(candidates.some((candidate) => candidate.edit.kind === 'add')).toBe(false);
  });

  it('offers cards from the pool', () => {
    const candidates = candidateEdits(deck(), REGISTRY, { pool: [CALM_UNIT.id] });
    expect(candidates.some((candidate) => candidate.edit.kind === 'add')).toBe(true);
  });
  ...
  it('refuses a card outside the Legend`s Domain Identity (103.1.b.4)', () => {
    ...
    const candidates = candidateEdits(deck(), registry, { pool: [OFF_IDENTITY.id] });
  ...
  it('respects the 3-copy limit (103.2.b)', () => {
    const candidates = candidateEdits(deck(), REGISTRY, { pool: [FURY_UNIT.id] });
    // The deck already runs 3.
    const adds = candidates.filter(
      (candidate) => candidate.edit.kind === 'add' && candidate.edit.card === FURY_UNIT.id,
    );
    expect(adds).toEqual([]);
  });
});
...
describe('describing an edit', () => {
  it('reads as an instruction a player could follow', () => {
    expect(
      describeEdit({ kind: 'cut', list: 'main', card: FURY_UNIT.id, count: 2 }, nameOf),
    ).toBe('Cut 2 Fury Unit');
    expect(
      describeEdit({ kind: 'add', list: 'main', card: CALM_UNIT.id, count: 1 }, nameOf),
    ).toBe('Add 1 Calm Unit');
    expect(
      describeEdit(
        { kind: 'swap', list: 'runes', out: CALM_RUNE.id, in: FURY_RUNE.id, count: 1 },
        nameOf,
      ),
