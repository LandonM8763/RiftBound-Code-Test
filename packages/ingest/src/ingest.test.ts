...
const fieldsOf = (gaps: readonly Gap[]): string[] =>
...
describe("domains", () => {
  it('treats "colorless" as the absence of a Domain, not a seventh one', () => {
    const { cards, problems } = run([
      raw({ domains: [{ id: "colorless", label: "Colorless" }] }),
    ]);
  ...
  it("reports a Domain it does not recognise", () => {
    const { cards, problems } = run([
      raw({ domains: [{ id: "shadow", label: "Shadow" }] }),
    ]);
...
describe("Runes", () => {
  it("takes the Rune Domain from its single printed Domain", () => {
    const { cards } = run([
      raw({ cardType: [{ id: "rune" }], domains: [{ id: "calm" }] }),
    ]);
...
describe("fields the source does not publish", () => {
  ...
  it("drops Spells rather than defaulting the timing to Action (rule 358.4)", () => {
    ...
    const { cards, gaps } = run([
      raw({ cardType: [{ id: "spell" }], energy: 1 }),
    ]);
    expect(cards).toEqual([]);
...
describe("rules text", () => {
  it("strips the source`s HTML markup", () => {
    ...
    expect(stripHtml("<p>Bold <em>and italic</em>.</p>")).toBe(
      "Bold and italic.",
    );
  });

  it("decodes HTML entities", () => {
    expect(stripHtml("<p>Ready &amp; exhaust &quot;me&quot;.</p>")).toBe(
      'Ready & exhaust "me".',
    );
  });
...
describe("gap summary", () => {
  ...
  it("names the card each gap belongs to", () => {
    const { gaps } = run([
      raw({ cardType: [{ id: "unit" }], name: "Blazing Scorcher", energy: 5 }),
    ]);
