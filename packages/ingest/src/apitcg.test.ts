...
describe("card types and supertypes", () => {
  it("reads the plain types", () => {
    for (const [input, type] of [
      ...
      expect(parseCardType(input)).toMatchObject({
        type,
        champion: false,
        signature: false,
      });
  ...
  it("reads the Signature supertype on any card type (133.7.b)", () => {
    expect(parseCardType("Signature Spell")).toMatchObject({
      type: "spell",
      signature: true,
    });
    expect(parseCardType("Signature Gear")).toMatchObject({
      type: "gear",
      signature: true,
    });
    expect(parseCardType("Signature Unit")).toMatchObject({
      type: "unit",
      signature: true,
    });
  });
  ...
  it("carries the supertypes onto the card", () => {
    const { cards } = run([
      ...
      raw({
        id: "origins-002-298",
        cardType: "Signature Spell",
        name: "Super Mega Death Rocket!",
        description: "ACTION",
      }),
    ]);
    expect(
      cards.find((card) => card.name === "Jinx - Demolitionist"),
    ).toMatchObject({
      champion: true,
      signature: false,
    });
    expect(
      cards.find((card) => card.name === "Super Mega Death Rocket!")?.signature,
    ).toBe(true);
  });
});
...
describe("Champion Tags", () => {
  ...
  it("ignores a printing qualifier when reading the tag", () => {
    expect(championTagFromName("Darius - Trifarian (Alternate Art)")).toBe(
      "Darius",
    );
  });
  ...
  it("133.8: records a gap when a card names a tag the export cannot supply", () => {
    ...
    const { cards, gaps } = run([
      raw({
        ...
        description: "Your Mechs have +1 Might.",
      }),
    ]);
    expect(cards[0]?.abilities?.statics?.[0]?.affects).toEqual({
      who: "friendly",
      tag: "Mech",
    });
    expect(gaps.find((gap) => gap.field === "tags")).toMatchObject({
      severity: "degraded",
    });
  });
  ...
  it("records no tags gap for a card that names none", () => {
    const { gaps } = run([
      raw({
        cardType: "Unit",
        name: "Plain",
        description: "Friendly units have +1 Might.",
      }),
    ]);
  ...
  it("records a gap for a Signature card whose tag is unreadable (103.2.d.2)", () => {
    ...
    expect(gaps.find((gap) => gap.field === "championTag")).toMatchObject({
      severity: "degraded",
    });
  });
});
...
describe("printings", () => {
  it("strips the qualifiers that mark a printing rather than a card", () => {
    for (const qualifier of [
      ...
      expect(canonicalName(`Vayne - Hunter (${qualifier})`)).toBe(
        "Vayne - Hunter",
      );
    }
  });
...
describe("Spell timing (rule 358.4)", () => {
  it("reads the printed marker", () => {
    expect(timingFromText("ACTION (Play on your turn or in showdowns.)")).toBe(
      "action",
    );
  ...
  it("drops a Spell rather than defaulting it to Action", () => {
    ...
    expect(gaps.find((gap) => gap.field === "timing")).toMatchObject({
      severity: "blocking",
    });
  });
  ...
  it("keeps a Spell whose timing is printed", () => {
    const { cards } = run([
      raw({
        cardType: "Spell",
        name: "Cleave",
        description: "ACTION <em>(Play on your turn.)</em>",
      }),
    ]);
...
describe("costs and Domains", () => {
  it("reads Energy and attributes Power pips to a single Domain", () => {
    const { cards } = run([
      raw({ energyCost: "3", powerCost: "2", domain: "Body" }),
    ]);
    expect((cards[0] as UnitCard).cost).toEqual({
      energy: 3,
      power: ["body", "body"],
    });
  });
  ...
  it("drops a multi-Domain card with Power rather than guessing the split", () => {
    const { cards, gaps } = run([
      raw({ domain: "Fury;Chaos", energyCost: "8", powerCost: "2" }),
    ]);
    expect(cards).toEqual([]);
    expect(gaps.find((gap) => gap.field === "cost.power")).toMatchObject({
      severity: "blocking",
    });
  });
  ...
  it("keeps a multi-Domain card that costs no Power", () => {
    const { cards } = run([
      raw({ domain: "Fury;Chaos", energyCost: "4", powerCost: "0" }),
    ]);
    expect(cards[0]?.domains).toEqual(["fury", "chaos"]);
  });

  it('treats "None" as the absence of a Domain', () => {
    const { cards, problems } = run([
      raw({ cardType: "Battlefield", domain: "None" }),
    ]);
...
describe("Might", () => {
  ...
  it("drops a Unit that has none (rules 465-466)", () => {
    ...
    expect(cards).toEqual([]);
    expect(gaps.find((gap) => gap.field === "might")).toMatchObject({
      severity: "blocking",
    });
  });
});
...
describe("non-cards in the export", () => {
  it("drops sealed products silently, since they are not gaps", () => {
    const { cards, gaps, dropped, problems } = run([
      ...
      {
        id: "origins-deck",
        name: "Origins - Champion Deck (Jinx)",
        cardType: "",
      },
    ]);
    expect(cards).toEqual([]);
  ...
  it("drops a bare Token, which is created rather than deckbuilt", () => {
    const { cards, problems } = run([
      raw({ id: "origins-357", cardType: "Token" }),
    ]);
    expect(cards).toEqual([]);
    expect(problems).toEqual([]);
  });
...
describe("rules text", () => {
  it("strips the export`s HTML", () => {
    expect(
      plainText("ACTION <em>(Play on your turn.)</em><br><br>\r\nDeal 3."),
    ).toBe("ACTION (Play on your turn.)\nDeal 3.");
  });
});
...
describe("output shape", () => {
  ...
  it("rejects input that is not an array", () => {
    expect(APITCG_SOURCE.normalize({ cards: [] }).problems[0]).toMatch(
      /array of cards/,
    );
  });
  ...
  it("summarizes its gaps by field", () => {
    ...
    expect(summarizeGaps(gaps).byField).toContainEqual({
      field: "timing",
      count: 2,
    });
  });
