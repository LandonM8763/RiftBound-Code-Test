...
import { isOver, playerLocation, type EntityId, type GameState } from './state.js';

const LEGEND = makeLegend(['fury'], { id: cardId('D-000') });
const CHAMPION = makeUnit(3, ['fury'], { id: cardId('D-001'), champion: true });
const PLAIN = makeUnit(2, ['fury'], { id: cardId('D-010'), name: 'Plain', cost: cost(1) });
...
const BONUS = makeUnit(2, ['fury'], {
  ...
  name: 'Bonus',
  cost: cost(1),
  abilities: { additionalCosts: [{ optional: true, pay: { kind: 'exhaustLegend' } }] },
  effect: {
    target: { kind: 'none' },
...
describe('the optional form (356.2.b)', () => {
  it('offers both paying and not paying, as separate plays', () => {
    ...
    expect(plays.filter((play) => 'payAdditional' in play && play.payAdditional === true)).toHaveLength(1);
  });
  ...
  it('changes the Total Cost, which is why the choice is made at step 2', () => {
    ...
    expect(totalCost(state, player, BARGAIN, { paidAdditionalCost: true })).toEqual(cost(2));
  });
  ...
  it('pays the cost and charges the discounted total', () => {
    ...
    const played = reduce(state, { type: 'playCard', card, payAdditional: true }).state;
  ...
  it('rejects a declaration on a card that has no optional cost', () => {
    ...
    expect(() => reduce(state, { type: 'playCard', card, payAdditional: true })).toThrow(
      IllegalActionError,
    );
  });
});
...
describe('the mandatory form (356.2.a)', () => {
  it('makes the card unplayable when the cost cannot be paid', () => {
    ...
    expect(() => reduce(state, { type: 'playCard', card })).toThrow(IllegalActionError);
  });
  ...
  it('is payable, and paid, once there is something to pay it with', () => {
    ...
    expect(canPayAdditional(state, player, { kind: 'kill', what: 'unit' })).toBe(true);
...
describe('payability', () => {
  it('checks each kind of payment against the state', () => {
    ...
    expect(canPayAdditional(state, player, { kind: 'discard', count: 1 })).toBe(true);
    expect(canPayAdditional(state, player, { kind: 'discard', count: 99 })).toBe(false);
    expect(canPayAdditional(state, player, { kind: 'exhaustLegend' })).toBe(true);
    expect(canPayAdditional(state, player, { kind: 'spendBuff' })).toBe(false);
    expect(canPayAdditional(state, player, { kind: 'kill', what: 'unit' })).toBe(false);
    expect(
  ...
  it('will not pay an exhaust cost twice', () => {
    ...
    const exhausted: GameState = {
      ...state,
      entities: { ...state.entities, [legend]: { ...state.entities[legend]!, exhausted: true } },
    };
