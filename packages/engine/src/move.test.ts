...
const UNIT = makeUnit(2, ['fury'], { id: cardId('M-010'), name: 'Mover', cost: cost(1) });
...
describe('the Standard Move (rule 144)', () => {
  it('moves a Unit from its Base to a Battlefield (rule 144.4.a)', () => {
    ...
    const after = reduce(state, { type: 'moveUnits', units: [unit], to: battlefieldLocation(0) })
      .state;
  ...
  it('refuses to move an exhausted Unit', () => {
    const [state, unit] = withReadyUnit(inMainPhase());
    const once = reduce(state, { type: 'moveUnits', units: [unit], to: battlefieldLocation(0) })
      .state;

    expect(() =>
      reduce(once, { type: 'moveUnits', units: [unit], to: playerLocation(once.activePlayer, 'base') }),
  ...
  it('moves several Units together as one action (rule 144.3)', () => {
    ...
    const after = reduce(state, { type: 'moveUnits', units, to: battlefieldLocation(0) }).state;
  ...
  it('rejects the same Unit twice in one Move', () => {
    const [state, unit] = withReadyUnit(inMainPhase());
    expect(() =>
      reduce(state, { type: 'moveUnits', units: [unit, unit], to: battlefieldLocation(0) }),
    ).toThrow(/cannot move twice/);
  });

  it('rejects an empty Move', () => {
    expect(() =>
      reduce(inMainPhase(), { type: 'moveUnits', units: [], to: battlefieldLocation(0) }),
  ...
  it('rejects moving a Unit another player controls', () => {
    ...
    const stolen: GameState = {
      ...state,
      entities: { ...state.entities, [unit]: { ...state.entities[unit]!, controller: opponent } },
    };

    expect(() =>
      reduce(stolen, { type: 'moveUnits', units: [unit], to: battlefieldLocation(0) }),
...
describe('Contested status (rules 190.3, 450)', () => {
  it('Contests an uncontrolled Battlefield when a Unit moves in', () => {
    ...
    const result = reduce(state, { type: 'moveUnits', units: [unit], to: battlefieldLocation(0) });
  ...
  it('blocks further movement while a Battlefield is Contested (rule 144.1.c)', () => {
    ...
    state = reduce(state, { type: 'moveUnits', units: [one], to: battlefieldLocation(0) }).state;
...
describe('Control cleanup (rule 190.4.c)', () => {
  it('loses Control of a Battlefield once the last Unit leaves', () => {
    ...
    expect(result.events).toContainEqual({ type: 'controlLost', battlefield: 0, player });
  });
...
describe('legal action generation', () => {
  it('offers a move for each ready Unit and destination', () => {
    const [state, unit] = withReadyUnit(inMainPhase());
    const moves = currentLegalActions(state).filter((action) => action.type === 'moveUnits');
  ...
  it('offers nothing for a Unit still in hand', () => {
    ...
    expect(currentLegalActions(state).filter((action) => action.type === 'moveUnits')).toEqual([]);
  });
