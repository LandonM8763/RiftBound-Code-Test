...
import { effectOf, type CardEffect, type TriggeredAbility } from '@riftbound/cards';

import { abilityFor, activatableAbilities, triggerCostPayable } from './abilities.js';
import { allTargetChoices, legalDestinations } from './effects.js';
...
export function legalActions(state: GameState, player: PlayerId): readonly Action[] {
  ...
  if (top !== undefined && top.pending && top.controller === player) {
    ...
    const choices = allTargetChoices(
      state,
      player,
      ability.effect,
      top.entity,
  ...
  for (const { source, index, ref, ability } of activatableAbilities(state, player)) {
    ...
    for (const targets of allTargetChoices(state, player, ability.effect, source)) {
      for (const destination of choicesOfDestination(state, player, ability.effect)) {
        actions.push({
  ...
  for (const { entity, check } of playableFromHand(state, player, { closed, showdown })) {
    ...
    for (const targets of allTargetChoices(state, player, effect, entity.id)) {
      ...
      // target rather than answered once for the card. A counted spec asks
      // about the first: every `choosesSource` modifier is an increase, so a
      // set that cannot afford its first member cannot afford the whole.
      if (affordable(state, player, check.card, check.payAdditional, targets[0]) === undefined) {
        continue;
      }
