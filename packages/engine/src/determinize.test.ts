...
function played(seed: string, steps: number): GameState {
  ...
  for (let i = 0; i < steps && !isOver(state); i += 1) {
    ...
    const actions =
      legal.length > 0 ? legal : legalActions(state, state.activePlayer);
    if (actions.length === 0) {
      break;
    }
  ...
  it("passes the engine`s own invariants", () => {
    for (const steps of [0, 20, 60, 140]) {
      ...
      const world = determinize(
        ...
        { definitions: DEFINITIONS, unseen: UNSEEN },
        "w",
      );
...
describe("what the viewer was entitled to see comes back exactly", () => {
  it("preserves the public board, the Chain and both trashes", () => {
    ...
    expect(world.battlefields.map((b) => b.units)).toEqual(
      state.battlefields.map((b) => b.units),
    );
  ...
  it("keeps entity ids, so an action chosen here is meaningful there", () => {
    ...
    for (const unit of state.battlefields.flatMap((b) => b.units)) {
      ...
      expect(world.entities[unit]!.controller).toBe(
        state.entities[unit]!.controller,
      );
    }
  });
});
...
describe("what the viewer was not entitled to see is sampled", () => {
  it("gets the counts right even though the cards are guesses", () => {
    ...
    expect(world.players[other]!.zones.hand.length).toBe(
      state.players[other]!.zones.hand.length,
    );
    for (const seat of state.players) {
      expect(world.players[seat.id]!.zones.mainDeck.length).toBe(
        seat.zones.mainDeck.length,
      );
      expect(world.players[seat.id]!.zones.runeDeck.length).toBe(
        seat.zones.runeDeck.length,
      );
    }
  });
  ...
  it("samples differently for different seeds, and the same for the same one", () => {
    ...
    const deckOf = (world: GameState): string =>
      JSON.stringify(
        world.players.map((p) =>
          p.zones.mainDeck.map((id) => world.entities[id]!.card),
        ),
      );
