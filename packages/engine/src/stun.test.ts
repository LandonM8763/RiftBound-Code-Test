...
describe("what Stun does and does not do (423.1.b, 423.1.c)", () => {
  ...
  it("423.1.c: killing it still takes damage equal to its full Might", () => {
    ...
    const hurt = withEntity(state, brute, (current) => ({
      ...current,
      damage: 3,
    }));
    expect(hasLethalDamage(hurt, brute)).toBe(false);
    const dead = withEntity(state, brute, (current) => ({
      ...current,
      damage: 4,
    }));
...
describe("the stun effect and its trigger (423.1.a)", () => {
  ...
  function jolt(seed: string): {
    ...
    return {
      state,
      unit: target,
      drawn: state.players[other]!.zones.hand.length - before,
    };
  }
...
describe("what each player can see (rule 274)", () => {
  it("Stunned is public, like Exhausted", () => {
    ...
    for (const viewer of [player, other] as const) {
      const seen = observe(state, viewer).battlefields[0]!.units.find(
        (u) => u.id === brute,
      );
...
describe("non-resource ability costs", () => {
  ...
  it("416.3: an empty trash makes the ability unusable", () => {
    ...
    expect(
      activatableAbilities(state, state.activePlayer).filter(
        (a) => a.source === unit,
      ),
    ).toHaveLength(0);
  });
  ...
  it("416.1: paying puts the card on the bottom of its owner`s Main Deck", () => {
    ...
    const offered = activatableAbilities(state, player).filter(
      (a) => a.source === unit,
    );
    ...
    expect(
      activatableAbilities(state, player).filter((a) => a.source === unit),
    ).toHaveLength(0);
    checkInvariants(state);
  });
