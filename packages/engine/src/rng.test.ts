...
describe('Rng determinism', () => {
  ...
  it('produces an identical stream for the same string seed', () => {
    expect(draw(Rng.fromSeed('game-1'), 20)).toEqual(draw(Rng.fromSeed('game-1'), 20));
  });

  it('produces different streams for different seeds', () => {
    expect(draw(Rng.fromSeed(1), 20)).not.toEqual(draw(Rng.fromSeed(2), 20));
    expect(draw(Rng.fromSeed('a'), 20)).not.toEqual(draw(Rng.fromSeed('b'), 20));
  });
  ...
  it('survives a JSON round trip of its state', () => {
    ...
    const restored = Rng.fromState(JSON.parse(JSON.stringify(original.state)) as never);
...
describe('Rng.shuffle', () => {
  ...
  it('is reproducible for a given seed', () => {
    const input = Array.from({ length: 40 }, (_, i) => i);
    expect(Rng.fromSeed('deck').shuffle(input)).toEqual(Rng.fromSeed('deck').shuffle(input));
  });
