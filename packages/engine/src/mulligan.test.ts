...
function deck(mainSize = 10): DeckList {
  return {
    ...
    main: Array.from(
      { length: mainSize },
      (_, i) => (UNITS[i % UNITS.length] as CardDefinition).id,
    ),
...
function newGame(seed = "mulligan", mainSize = 10): GameState {
  return createGame({
    decks: [deck(mainSize), deck(mainSize)],
    registry: REGISTRY,
    seed,
  }).state;
}
...
describe("the Mulligan (rule 117)", () => {
  ...
  it("rejects setting aside more than the limit (117.1)", () => {
    ...
    expect(() => reduce(state, { type: "mulligan", cards: tooMany })).toThrow(
      IllegalActionError,
    );
  });
  ...
  it("rejects a card that is not in the mulliganing player`s hand", () => {
    ...
    expect(() => reduce(state, { type: "mulligan", cards: [theirs] })).toThrow(
      IllegalActionError,
    );
  });
  ...
  it("rejects the same card set aside twice", () => {
    ...
    expect(() =>
      reduce(state, { type: "mulligan", cards: [card, card] }),
  ...
  it("cannot be taken once play has begun", () => {
    ...
    state = take(state, []);
    state = take(state, []);
    expect(() => reduce(state, { type: "mulligan", cards: [] })).toThrow(
      IllegalActionError,
    );
  });
