...
describe("rule 356.1: base cost modifications", () => {
  it("replaces the Base Cost outright (356.1.a)", () => {
    const result = apply(
      cost(5, "fury"),
      mods(SEAT_0, { kind: "replaceBase", cost: cost(1) }),
    );
  ...
  it("sets both components to zero when the cost is ignored (356.1.b.1)", () => {
    const result = apply(
      cost(4, "fury", "calm"),
      mods(SEAT_0, { kind: "ignoreBase", component: "both" }),
    );
  ...
  it("ignores only the named component (356.1.b.2)", () => {
    expect(
      apply(
        cost(4, "fury"),
        mods(SEAT_0, { kind: "ignoreBase", component: "energy" }),
      ),
    ).toEqual({ energy: 0, power: ["fury"] });
    expect(
      apply(
        cost(4, "fury"),
        mods(SEAT_0, { kind: "ignoreBase", component: "power" }),
      ),
    ).toEqual({ energy: 4, power: [] });
  });
  ...
  it("lets a later layer raise an ignored cost back above zero (356.1.b.3)", () => {
    ...
    const result = apply(
      ...
      mods(
        SEAT_0,
        { kind: "ignoreBase", component: "both" },
        { kind: "increase", energy: 1, power: ["fury"] },
      ),
    );
...
describe("rule 356.3: cost increases", () => {
  it("raises the Energy cost", () => {
    expect(
      apply(cost(2), mods(SEAT_0, { kind: "increase", energy: 2 })),
    ).toEqual(cost(4));
  });

  it("adds Power pips", () => {
    expect(
      apply(cost(1), mods(SEAT_0, { kind: "increase", power: ["calm"] })),
    ).toEqual(cost(1, "calm"));
  });
  ...
  it("applies before discounts, not after (356.3 then 356.4)", () => {
    ...
    const result = apply(
      ...
      mods(
        SEAT_0,
        { kind: "discount", energy: 1 },
        { kind: "increase", energy: 2 },
      ),
    );
...
describe("rule 356.4: discounts", () => {
  it("reduces the Energy cost", () => {
    expect(
      apply(cost(5), mods(SEAT_0, { kind: "discount", energy: 2 })),
    ).toEqual(cost(3));
  });
  ...
  it("removes a Power pip of the named Domain", () => {
    expect(
      apply(
        cost(1, "fury", "calm"),
        mods(SEAT_0, { kind: "discount", power: ["fury"] }),
      ),
    ).toEqual(cost(1, "calm"));
  });
  ...
  it("cannot reduce a cost below zero (356.6)", () => {
    expect(
      apply(cost(2), mods(SEAT_0, { kind: "discount", energy: 5 })),
    ).toEqual(cost(0));
  });
  ...
  it("respects its own minimum (356.4.e)", () => {
    expect(
      apply(
        cost(8),
        mods(SEAT_0, { kind: "discount", energy: 1, minimumEnergy: 1 }),
      ),
    ).toEqual(cost(7));
    expect(
      apply(
        cost(3),
        mods(SEAT_0, { kind: "discount", energy: 5, minimumEnergy: 1 }),
      ),
    ).toEqual(cost(1));
  });
  ...
  it("never raises a cost that is already under the minimum", () => {
    ...
    expect(
      apply(
        cost(0),
        mods(SEAT_0, { kind: "discount", energy: 1, minimumEnergy: 2 }),
      ),
    ).toEqual(cost(0));
  });
...
describe("which cards a modifier applies to", () => {
    ...
    const spellsOnly: ActiveModifier[] = [
      {
        controller: SEAT_0,
        modifier: {
          applies: { scope: "any", types: ["spell"] },
          change: discount,
        },
      },
    ];
    expect(applyModifiers(cost(3), "spell", SEAT_0, spellsOnly)).toEqual(
      cost(2),
    );
    expect(applyModifiers(cost(3), "unit", SEAT_0, spellsOnly)).toEqual(
      cost(3),
    );
  });
  ...
  it("an empty type list means every type", () => {
    const anyType: ActiveModifier[] = [
      {
        controller: SEAT_0,
        modifier: { applies: { scope: "any", types: [] }, change: discount },
      },
    ];
  ...
  it("a friendly-scoped modifier helps only its own controller", () => {
    const friendly: ActiveModifier[] = [
      {
        controller: SEAT_0,
        modifier: { applies: { scope: "friendly" }, change: discount },
      },
    ];
...
describe("ability costs (rule 403)", () => {
    ...
    const forAbilities: ActiveModifier[] = [
      {
        controller: SEAT_0,
        modifier: {
          applies: { scope: "friendly", types: ["ability"] },
          change: discount,
        },
      },
    ];
    expect(applyModifiers(cost(2), "ability", SEAT_0, forAbilities)).toEqual(
      cost(1),
    );
  });
  ...
  it("are untouched by an unqualified card modifier", () => {
    ...
    expect(applyModifiers(cost(2), "ability", SEAT_0, forCards)).toEqual(
      cost(2),
    );
  ...
  it("are untouched by a modifier naming only card types", () => {
    ...
    expect(applyModifiers(cost(2), "ability", SEAT_0, unitsOnly)).toEqual(
      cost(2),
    );
  });
});
...
function mint(
  ...
  return [
    zone === "base"
      ? placed
      : moveEntity(placed, entity, playerLocation(player, zone)),
    entity,
  ];
}
...
describe("cost modification in play", () => {
  ...
  it("makes a card playable that the pool could not otherwise afford", () => {
    ...
    const withoutDiscount = withCard(
      inMainPhase("afford", 2),
      EXPENSIVE.id,
      "hand",
    );
    const before = legalActions(
      withoutDiscount[0],
      withoutDiscount[0].activePlayer,
    ).filter(
      (action) =>
        action.type === "playCard" && action.card === withoutDiscount[1],
    );
...
describe('self cost modifiers ("I cost 2 less")', () => {
  ...
  it("discounts the card it is printed on, from hand", () => {
    ...
    expect(totalCost(state, state.activePlayer, SELF_DISCOUNT)).toEqual(
      cost(1),
    );
  });
  ...
  it('counts cards in the trash (356.4, "for each")', () => {
    ...
    for (const expected of [3, 2, 1]) {
      ...
      expect(totalCost(state, state.activePlayer, TRASH_SCALER)).toEqual(
        cost(expected),
      );
    }
  });
  ...
  it("withholds a Legion-gated discount until another card has been played", () => {
    ...
    const after: GameState = {
      ...
      players: state.players.map((seat) =>
        seat.id === player
          ? { ...seat, playedThisTurn: [0 as EntityId] }
          : seat,
      ),
    };
