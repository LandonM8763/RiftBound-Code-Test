...
function resolveTrigger(
  ...
  // A pending item is either an Ability or 390.5's Delayed Linked Ability,
  // which carries its effect inline because it is on no card's ability list.
  const linked = top.linked;
  const ability =
    linked !== null ? undefined : top.ability === null
      ? undefined
      : abilityFor(state, top.entity, top.ability);
  if (linked === null && ability === undefined) {
    throw new IllegalActionError('The pending ability no longer exists');
  }
  const effect = linked ?? (ability as ActivatedAbility | TriggeredAbility).effect;
  ...
  if (!perform) {
    ...
    // pauses here to make choices, not to be refused. A Linked Ability is
    // never optional in itself — "you may play one" is a `min: 0` on its
    // target, which is a choice rather than a refusal.
    if (ability === undefined || !('optional' in ability) || ability.optional !== true) {
  ...
  // 390.5's Linked Ability has no cost of its own.
  let next = ability === undefined ? state : payTriggerCost(state, player, ability, top.entity, events);
...
/**
 * 390.5: put a Delayed Linked Ability on the Chain.
 *
 * Generated by a `look`, and carrying the cards it looked at. It is Pending
 * exactly when it has something to settle at 402.2 — a choice among those cards
 * — so a Linked Ability that chooses nothing goes straight on and resolves in
 * turn, and one that does waits for its controller the way a Triggered Ability
 * does. That reuse is the point: no decision point *during* resolution is
 * needed, because the choice is a step of playing the linked ability.
 */
function queueLinked(
  ...
  const pending = needsTargetChoice(effect.target) || needsDestination(effect);
  ...
  events.push({ type: 'chainItemAdded', entity: source, controller });
  // 312.2.c: a Pending item needs its controller to finalize it before anyone
  // else may act.
  return pending ? { ...next, priority: controller } : next;
}
...
const EFFECT_CONTEXT: EffectContext = {
  ...
  queueLinked: (state, controller, source, effect, revealed, revealedToAll, events) =>
    queueLinked(state, controller, source, effect, revealed, revealedToAll, events),
};
...
function pass(state: GameState): ReduceResult {
  ...
  // 390.5: a Delayed Linked Ability carries its effect inline — it is not on
  // the source card's ability list, so there is no index to look it up by.
  if (top.linked !== null) {
    next = executeEffect(
      next,
      {
        ...
        // 424.1.a.2: the cards it looked at, still where they were.
