  ...
  return {
    ...
    players: state.players.map((seat) =>
      seat.id === player
        ? {
            ...seat,
            pool: { ...seat.pool, power: { ...seat.pool.power, fury: amount } },
          }
        : seat,
    ),
  };
...
describe("the Hide action (rule 421)", () => {
  ...
  it("811.1.b: costs one [A], Power of any Domain", () => {
    ...
    expect(
      legalActions(withCard, player).some((action) => action.type === "hide"),
    ).toBe(false);
    expect(() =>
      reduce(withCard, { type: "hide", card: lurker, battlefield: 0 }),
    ).toThrow();
  ...
  it("107.3.b: one card per Facedown Zone", () => {
    ...
    expect(() =>
      reduce(state, { type: "hide", card: second, battlefield: 0 }),
    ).toThrow();
  });
  ...
  it("refuses a card without the keyword", () => {
    ...
    expect(() =>
      reduce(withCard, { type: "hide", card: plain, battlefield: 0 }),
    ).toThrow();
  });
...
describe("playing from facedown (811.1.b, 811.1.d)", () => {
  ...
  function hidden(
    seed: string,
    id: CardDefinition["id"] = LURKER.id,
  ): [GameState, EntityId] {
    ...
    return [
      reduce(withCard, { type: "hide", card, battlefield: 0 }).state,
      card,
    ];
  }
  ...
  it("811.1.b: not on the turn it went down", () => {
    ...
    expect(playableFromFacedown(state, state.activePlayer)).not.toContain(
      lurker,
    );
  ...
  it("811.1.d.2: may only choose at the Battlefield it was hidden at", () => {
    ...
    const offered = legalActions(later, player)
      ...
      .map((action) => (action as { targets?: EntityId[] }).targets?.[0]);
    expect(offered).toContain(here);
    expect(offered).not.toContain(elsewhere);
    expect(() =>
      reduce(later, { type: "playCard", card: ambush, targets: [elsewhere] }),
    ).toThrow();
  });
  ...
  it("811.3: the card can still be played from hand for its cost", () => {
    ...
    expect(
      legalActions(state, player).some((action) => action.type === "playCard"),
    ).toBe(true);
  });
});
...
describe("Predict (436) and Vision (817)", () => {
  ...
  it("436.4.a: an empty deck Predicts nothing and is not a Burn Out", () => {
    ...
    state = {
      ...
      players: state.players.map((seat) =>
        seat.id === player
          ? { ...seat, zones: { ...seat.zones, mainDeck: [] } }
          : seat,
      ),
    };
