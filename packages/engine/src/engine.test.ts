...
describe("createGame", () => {
  ...
  it("selects each player's Battlefield from the three they brought (rule 485.5)", () => {
    const decks = [testDeck(), testDeck()];
    const placed = createGame({ decks, registry: registryFor(), seed: "bf" })
      .state.battlefields;
  ...
  it("picks Battlefields at random, not always the first", () => {
    ...
    const chosen = new Set(
      Array.from(
        { length: 40 },
        (_, i) =>
          createGame({ decks, registry: registryFor(), seed: `bf-${i}` }).state
            .battlefields[0]?.card,
      ),
    );
  ...
  it("rejects a game with fewer than two players", () => {
    ...
    expect(() =>
      createGame({ decks, registry: registryFor(), seed: 1 }),
  ...
  it("rejects a player who brought no Battlefields", () => {
    ...
    expect(() =>
      createGame({ decks: [thin, thin], registry: registryFor(), seed: 1 }),
    ).toThrow(/no Battlefields/);
  });
...
describe("determinism", () => {
  ...
  it("produces a different game for a different seed", () => {
    const registry = registryFor();
    expect(
      JSON.stringify(createGame({ decks, registry, seed: "a" }).state),
    ).not.toEqual(
  ...
  it("replays an identical game for the same seed and action sequence", () => {
    const play = (): GameState => {
      let state = pastMulligan(
        createGame({ decks, registry: registryFor(), seed: "replay" }).state,
      );
...
describe("scoring (rules 467-471)", () => {
  ...
  it("records a Hold as the Scoring method (rule 469.2)", () => {
    ...
    const player = start.activePlayer;
    const result = reduce(
      reduce(holding(start, player), { type: "resolvePhase" }).state,
      {
        type: "resolvePhase",
      },
    );
  ...
  it("wins on reaching the Victory Score with more points than the opponent", () => {
    ...
    const brink: GameState = {
      ...
      players: start.players.map((current) =>
        current.id === player
          ? { ...current, points: start.config.victoryScore - 1 }
          : current,
      ),
    ...
    const finished = reduce(reduce(brink, { type: "resolvePhase" }).state, {
      type: "resolvePhase",
    });
...
describe("Burn Out (rules 413.4, 431)", () => {
  ...
  it("burns out when the Main Deck is empty at the Draw Phase", () => {
    const start = pastMulligan(
      createGame({ decks: emptyDecks(), registry: registryFor(), seed: "burn" })
        .state,
    );
    const player = start.activePlayer;
  ...
  it("recycles the trash back into the Main Deck (rule 431.2.b)", () => {
    ...
    for (const id of hand) {
      entities[id] = {
        ...entities[id]!,
        location: { kind: "player", player, zone: "trash" },
      };
    }
    const trashed: GameState = {
      ...
      players: start.players.map((p) =>
        p.id === player
          ? { ...p, zones: { ...p.zones, hand: [], trash: [...hand] } }
          : p,
      ),
      entities,
    };
    ...
    const after = reduce(runTo(trashed, "draw"), {
      type: "resolvePhase",
    }).state;
...
describe("legal actions", () => {
  ...
  it("offers endTurn during the Main Phase, alongside the Add abilities", () => {
    const actions = currentLegalActions(
      runTo(newGame("legal-main").state, "main"),
    );
...
describe("illegal actions", () => {
  ...
  it("rejects any action once the game is over", () => {
    ...
    expect(() => reduce(ended, { type: "resolvePhase" })).toThrow(
      IllegalActionError,
    );
  });
  ...
  it("rejects an unknown action", () => {
    expect(() =>
      reduce(newGame("illegal-unknown").state, {
        type: "nonsense",
